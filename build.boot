(set-env!
 :source-paths    #{"src/cljs" "src/trackers"}
...
(deftask run
  ...
  (comp (serve)
        ...
        (reload)
        (build)
        (target :dir #{"out"}))) ;; target added to understand what's happening in the filesystem
...
(deftask development []
  (task-options! cljs {:optimizations :simple}
                 reload {:on-jsload 'code-splitting-bug-boot-cljs.app/init})
  identity)
